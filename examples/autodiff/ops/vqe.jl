...
gradtol = 1.0e-4 # Tolerance for stopping gradient descent
...
function variational_circuit(nsites, nlayers, θ⃗)
    ...
    circuit = layer(nsites, θ⃗[range])
    for n in 1:(nlayers - 1)
        circuit = layer(nsites, θ⃗[range .+ n * nsites]) * circuit
    end
    return circuit
end
