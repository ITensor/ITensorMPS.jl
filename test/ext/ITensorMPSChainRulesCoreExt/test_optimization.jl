...
@testset "optimization" begin
    @testset "Energy minimization" begin
        ...
        linesearch = HagerZhangLineSearch(;
            c₁ = 0.1, c₂ = 0.9, ϵ = 1.0e-6, θ = 1 / 2, γ = 2 / 3, ρ = 5.0, verbosity = 0
        )
        algorithm = LBFGS(3; maxiter = 20, gradtol = 1.0e-8, linesearch = linesearch)
    ...
    @testset "Energy minimization (MPS)" begin
        ...
        @test E(H, ψ) ≈ inner(ψmps', Hmpo, ψmps) / inner(ψmps, ψmps) rtol = 1.0e-2
    end
    ...
    @testset "State preparation (full state)" begin
        ...
        @test loss(θ⃗ₒₚₜ) ≈ loss(θ⃗) rtol = 1.0e-2
    end
    ...
    @testset "State preparation (MPS)" begin
        for gate in ["Ry"]
            ...
            gradtol = 1.0e-3 # Tolerance for stopping gradient descent
            ...
            ψᵗᵃʳᵍᵉᵗ = apply(Uᵗᵃʳᵍᵉᵗ, ψ0; cutoff = 1.0e-8)
            ...
            function loss(θ⃗)
                ...
                Uθ⃗ = ops(𝒰θ⃗, s)
                ψθ⃗ = apply(Uθ⃗, ψ0; cutoff = 1.0e-8)
            ...
            @test loss(θ⃗ₒₚₜ) ≈ loss(θ⃗ᵗᵃʳᵍᵉᵗ) rtol = 1.0e-5
        end
    end
    ...
    @testset "VQE (MPS)" begin
        ...
        function loss(θ⃗)
            ...
            Uθ⃗ = ops(𝒰θ⃗, s)
            ψθ⃗ = apply(Uθ⃗, ψ0; cutoff = 1.0e-8)
            return inner(ψθ⃗', H, ψθ⃗; cutoff = 1.0e-8)
        end
        ...
        @test loss(θ⃗ₒₚₜ) ≈ e_dmrg rtol = 1.0e-1
    end
end
