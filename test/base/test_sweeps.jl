...
@testset "Sweeps constructor" begin
    ...
    sweep_args = [
        ...
        50 10 1.0e-12 1.0e-7
        100 20 1.0e-12 1.0e-8
        200 20 1.0e-12 1.0e-10
        400 20 1.0e-12 0
        800 20 1.0e-12 1.0e-11
        800 20 1.0e-12 0
    ]
    ...
    @testset "Don't specify nsweep" begin
        ...
        for n in 1:nsw
            @test cutoff(sw, n) == 1.0e-12
        end

        @test noise(sw, 1) == 1.0e-7
        @test noise(sw, 2) == 1.0e-8
        @test noise(sw, 3) == 1.0e-10
        @test noise(sw, 4) == 0
        @test noise(sw, 5) == 1.0e-11
        @test noise(sw, 6) == 0
    end
    ...
    @testset "Variable types of input" begin
        sw = Sweeps(5)
        setnoise!(sw, 1.0e-8, 0)
        @test noise(sw, 1) ≈ 1.0e-8
        @test noise(sw, 2) ≈ 0.0
        @test noise(sw, 3) ≈ 0.0
        setcutoff!(sw, 0, 1.0e-8, 0, 1.0e-12)
        @test cutoff(sw, 1) ≈ 0.0
        @test cutoff(sw, 2) ≈ 1.0e-8
        @test cutoff(sw, 3) ≈ 0.0
        @test cutoff(sw, 4) ≈ 1.0e-12
    end

    @testset "Keyword args to constructor" begin
        sw = Sweeps(5; maxdim = [4, 8, 16], mindim = 1, cutoff = [1.0e-5, 1.0e-8])
        @test maxdim(sw, 1) == 4
        @test maxdim(sw, 2) == 8
        @test maxdim(sw, 3) == 16
        @test maxdim(sw, 4) == 16
        ...
        @test cutoff(sw, 1) ≈ 1.0e-5
        @test cutoff(sw, 2) ≈ 1.0e-8
        @test cutoff(sw, 3) ≈ 1.0e-8
        @test cutoff(sw, 4) ≈ 1.0e-8
        @test cutoff(sw, 5) ≈ 1.0e-8

        sw = Sweeps(5; cutoff = 1.0e-8)
