...
@testset "MPS Basics" begin
    ...
    @testset "normalize/normalize! MPS" begin
        ...
        @test norm(psi) ≈ 1.0
        @test lognorm(psi) ≈ 0.0 atol = 1.0e-15
        ...
        @test norm(phi) ≈ 1
        @test lognorm(phi) ≈ 0 atol = 1.0e-14
    end
...
function test_correlation_matrix(psi::MPS, ops::Vector{Tuple{String, String}}; kwargs...)
    ...
    for op in ops
        ...
        @test Cpm ≈ Copsum rtol = 1.0e-11

        PM = expect(psi, op[1] * " * " * op[2])
        @test norm(PM - diag(Cpm)) < 1.0e-8
    end
    return
end
...
@testset "MPS gauging and truncation" begin
    ...
    @testset "truncate! with callback" begin
        ...
        truncate!(mps_; maxdim = 3, cutoff = 1.0e-3, callback = _callback)
...
@testset "Other MPS methods" begin
    ...
    @testset "Expected value and Correlations" begin
        ...
        @test norm(Cpm37 - Cpm[range, range]) < 1.0e-8
    ...
    @testset "swapbondsites" begin
        ...
        ψ0 = random_mps(sites)
        ψ = replacebond(ψ0, 3, ψ0[3] * ψ0[4]; swapsites = true, cutoff = 1.0e-15)
        ...
        ψ = swapbondsites(ψ0, 4; cutoff = 1.0e-15)
        @test siteind(ψ, 1) == siteind(ψ0, 1)
        @test siteind(ψ, 2) == siteind(ψ0, 2)
    ...
    @testset "movesites $N sites" for N in 1:4
        ...
        for perm in permutations(1:N)
            ...
            @test ns′ == perm
            ψ′ = movesites(ψ, 1:N .=> ns′; cutoff = 1.0e-15)
            if N == 1
                @test maxlinkdim(ψ′) == 1
            else
                @test maxlinkdim(ψ′) == 1
    ...
    @testset "Construct MPS from ITensor" begin
        ...
        A = prod(ψ0)
        ψ = MPS(A, s; cutoff = 1.0e-15, orthocenter = 2)
        @test prod(ψ) ≈ A
    ...
    @testset "movesites subsets of sites" begin
        ...
        for i in 1:N, j in 1:N
            ...
            ψ′ = movesites(ψ, ns .=> ns′; cutoff = 1.0e-15)
            @test siteind(ψ′, min_ns) == siteind(ψ, i)
            @test siteind(ψ′, min_ns + 1) == siteind(ψ, j)
            @test maxlinkdim(ψ′) == 1
            ψ̃ = movesites(ψ′, ns′ .=> ns; cutoff = 1.0e-15)
            for n in 1:N
    ...
    @testset "movesites tags" begin
        ...
        for perm in permutations(1:N)
            s = s0[perm]
            ns′ = [findfirst(==(i), s0) for i in s]
            ψ = movesites(ψ0, 1:N .=> ns′; cutoff = 1.0e-15)
    ...
    @testset "product(::Vector{ITensor}, ::MPS)" begin
        ...
        for i in 1:N, j in 1:N
            ...
            CXij_ψ = product([X[i], CX[i, j]], ψ; move_sites_back = false, cutoff = 1.0e-15)
        ...
        for i in 1:N, j in 1:N, k in 1:N
            ...
            # Don't move sites back
            CCNOTijk_ψ = product(
                [X[j], X[i], CCNOT[ns...]], ψ; move_sites_back = false, cutoff = 1.0e-15
            )
            @test maxlinkdim(CCNOTijk_ψ) == 1
            @test prod(CCNOTijk_ψ) ≈ prod(MPS(s, n -> n ∈ ns ? "1" : "0"))

            # Move sites back
            CCNOTijk_ψ = product([X[j], X[i], CCNOT[ns...]], ψ; cutoff = 1.0e-15)
            @test maxlinkdim(CCNOTijk_ψ) == 1
            for n in 1:N
        ...
        for i in 1:N, j in i:N, k in 1:N, l in k:N
            ...
            # Move sites back
            CCCNOTijkl_ψ = product([X[i], X[j], X[k], CCCNOT[ns...]], ψ; cutoff = 1.0e-15)
            @test maxlinkdim(CCCNOTijkl_ψ) == 1
            for n in 1:N
    ...
    @testset "product" begin
        ...
        @testset "More complex evolution" begin
            ...
            @testset "Pure state evolution" begin
                ψ0 = MPS(s, "0")
                ψ = product(gates, ψ0; cutoff = 1.0e-15)
                @test maxlinkdim(ψ) == 8
                prodψ = product(gates, prod(ψ0))
                @test prod(ψ) ≈ prodψ rtol = 1.0e-12
            end
            ...
            @testset "Mixed state evolution" begin
                M = product(gates, M0; cutoff = 1.0e-15, maxdim = maxdim)
        ...
        @testset "Gate evolution state" begin
            ...
            # Apply the gates
            ψ = apply(gates, ψ0; cutoff = 1.0e-15, maxdim = 100)

            prodψ = apply(gates, prod(ψ0))
            @test prod(ψ) ≈ prodψ rtol = 1.0e-4
        end
        ...
        @testset "With fermions" begin
            ...
            # Results in -|110⟩
            ψ1 = product(Os, ψ011; cutoff = 1.0e-15)
            ...
            # Results in -|110⟩
            ψ2 = noprime(contract(H, ψ011; cutoff = 1.0e-15))
        ...
        @testset "Spinless fermion (gate evolution)" begin
            ...
            for i in 1:(N - 1), j in (i + 1):N
                ...
                A_OP = prod(product(G1, ψ0; cutoff = 1.0e-6))

                A_OPS = noprime(G2 * prod(ψ0))

                A_MPO = noprime(prod(contract(G3, ψ0; cutoff = 1.0e-6)))
            ...
            for i in 1:(N - 3), j in (i + 1):(N - 2), k in (j + 1):(N - 1), l in (k + 1):N
                ...
                @disable_warn_order begin
                    ...
                    A_OP = prod(product(G1, ψ0; cutoff = 1.0e-16))

                    A_OPS = noprime(G2 * prod(ψ0))

                    A_MPO = noprime(prod(contract(G3, ψ0; cutoff = 1.0e-16)))
                end
                @test A_OPS ≈ A_OP rtol = 1.0e-12
                @test A_MPO ≈ A_OP rtol = 1.0e-12
            end
        end
        ...
        @testset "Spinful Fermions (Electron) gate evolution" begin
            ...
            for i in 1:(N - 1), j in (i + 1):N
                ...
                G1 = MPO(opsum, s)
                G2 = op("CCup", s, i, j)
                A_MPO = prod(noprime(contract(G1, ψ; cutoff = 1.0e-8)))
                A_OP = prod(product(G2, ψ; cutoff = 1.0e-8))
                @test A_MPO ≈ A_OP atol = 1.0e-4
            end
        end
    end
    ...
    @testset "orthogonalize! on MPS with no link indices" begin
        ...
        @test ITensorMPS.dist(ψ, ϕ) ≈ 0 atol = 1.0e-6
        # TODO: use this instead?
        # @test lognorm(ψ - ϕ) < -16
        @test norm(ψ - ϕ) ≈ 0 atol = 1.0e-6
    end
    ...
    @testset "Truncate MPO with no link indices" begin
        ...
        Mt = truncate(M; cutoff = 1.0e-15)
        @test ITensorMPS.hasdefaultlinktags(Mt)
        @test norm(M - Mt) ≈ 0 atol = 1.0e-12
    end
end
end
