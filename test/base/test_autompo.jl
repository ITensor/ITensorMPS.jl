...
@testset "OpSum" begin
    ...
    @testset "Chemical Hamiltonian Test" begin
        for auto_fermion in [false, true]
            ...
            for i in 1:N, j in 1:N
                ...
                psiᵢ = MPS(s, stᵢ)
                psiⱼ = MPS(s, stⱼ)
                @test abs(inner(psiᵢ', Ht, psiⱼ) - t[i, j]) < 1.0e-10
            end
            ...
            for i in 1:N, j in 1:N, k in 1:N, l in 1:N
                ...
                (k > l) && (exact_val *= -1)
                (i > j) && (exact_val *= -1)
                @test abs(mpo_val - exact_val) < 1.0e-10
            end
        end
        ITensors.disable_auto_fermion()
    end
    ...
    @testset "Accuracy Regression Test (Issue 725)" begin
        ...
        @test abs(inner(psi0', H, psi0) - 0.00018) < 1.0e-10
    end
