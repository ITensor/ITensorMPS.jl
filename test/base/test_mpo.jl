...
@testset "MPO Basics" begin
    ...
    @testset "error_contract" begin
        ...
        Kphi = contract(K, phi; method = "naive", cutoff = 1.0e-8)
    ...
    @testset "add(::MPO, ::MPO)" begin
        ...
        @test inner(psi', k_psi + l_psi) ≈ ⋅(psi', M, psi) atol = 5.0e-3
        @test inner(psi', sum([k_psi, l_psi])) ≈ dot(psi', M, psi) atol = 5.0e-3
        for dim in 2:4
            shsites = siteinds("S=1/2", N)
            ...
            @test inner(psi', k_psi + l_psi) ≈ dot(psi', M, psi) atol = 5.0e-3
            @test inner(psi', sum([k_psi, l_psi])) ≈ inner(psi', M, psi) atol = 5.0e-3
            psi = random_mps(shsites)
            M = add(K, L; cutoff = 1.0e-9)
            k_psi = contract(K, psi)
            l_psi = contract(L, psi)
            @test inner(psi', k_psi + l_psi) ≈ inner(psi', M, psi) atol = 5.0e-3
        end
    end
    ...
    @testset "*(::MPO, ::MPO)" begin
        ...
        @test ⋅(psi'', KL, psi) ≈ dot(psi'', psi_kl_out) atol = 5.0e-3
        ...
        @test dot(psik, KL, psil) ≈ psik ⋅ psi_kl_out atol = 5.0e-3
    ...
    @testset "Multi-arg apply(::MPO...)" begin
        ...
        @test apply(ρ1, ρ2, ρ3; cutoff = 1.0e-8) ≈
            apply(apply(ρ1, ρ2; cutoff = 1.0e-8), ρ3; cutoff = 1.0e-8)
    end
    ...
    @testset "Construct MPO from ITensor" begin
        ...
        ψ0 = MPO(s, "Id")
        A = prod(ψ0)
        ψ = MPO(A, sis; cutoff = 1.0e-15, orthocenter = 3)
        ls = linkinds(ψ)
        @test hassameinds(ψ[1], (s[1], s[1]', ls[1]))
        @test hassameinds(ψ[N], (s[N], s[N]', ls[N - 1]))
        @test prod(ψ) ≈ A
        ...
        ψ0 = MPO(s, "Id")
        A = prod(ψ0)
        ψ = MPO(A, s; cutoff = 1.0e-15, orthocenter = 3)
        ls = linkinds(ψ)
        @test hassameinds(ψ[1], (s[1], s[1]', ls[1]))
        @test hassameinds(ψ[N], (s[N], s[N]', ls[N - 1]))
        @test prod(ψ) ≈ A
    ...
    @testset "outer(::MPS, ::MPS) and projector(::MPS)" begin
        ...
        Pψ = projector(ψ; normalize = false, cutoff = 1.0e-8)
        ...
        ) / sqrt(inner(ψ, ψ) * inner(ϕ, ϕ)) ≈ 0 atol = 1.0e-5 * N
    end
    ...
    @testset "MPO with no link indices" for conserve_qns in [false, true]
        ...
        Htrunc = truncate(H; cutoff = 1.0e-8)
        @test Htrunc ≈ H
        @test linkdims(Htrunc) == fill(1, length(s) - 1)

        H² = apply(H, H; cutoff = 1.0e-8)
