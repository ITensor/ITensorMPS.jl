...
@testset "Basic DMRG" begin
    @testset "Spin-one Heisenberg" begin
        ...
        mindim!(sweeps, 1, 10)
        cutoff!(sweeps, 1.0e-11)
        noise!(sweeps, 1.0e-10)
    ...
    @testset "Transverse field Ising" begin
        ...
        @test abs((energy - energy_exact) / energy_exact) < 1.0e-4
    end
    ...
    @testset "DMRGObserver" begin
        ...
        observer = DMRGObserver(; minsweeps = 2, energy_tol = 1.0e-4)
        ...
        @test all(truncerrors(observer) .< 1.0e-9)
